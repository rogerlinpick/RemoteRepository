...
public class AdminDAO implements Admin_InterfaceDAO{
	...
	public static void main(String[] args) {		
		...
		bean.setAdmin_id(7);
		bean.setAdmin_account("Mary");
//		bean.setLevel_num(level);
...
//		dao.update(bean);
	...
	@Override
	public boolean update(AdminVO bean) {
		Session session = getSession();
		AdminVO temp = (AdminVO)session.get(AdminVO.class, bean.getAdmin_id());
		if(temp!=null){			
			temp.setAdmin_id(bean.getAdmin_id());
			temp.setAdmin_account(bean.getAdmin_account());
			temp.setAdmin_first_name(bean.getAdmin_first_name());
			temp.setAdmin_last_name(bean.getAdmin_last_name());
			temp.setAdmin_pic(bean.getAdmin_pic());
			temp.setLevel_num(bean.getLevel_num());
